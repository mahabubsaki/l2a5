...
const Transition = ({ children, item }: { children: ReactNode; item: string; }) => {
    ...
    return (
        <motion.div initial={{ overflow: 'hidden' }} ref={scope} >
            <motion.div ref={rootRef} initial={{ y: '100%', opacity: 0 }} className="min-h-screen">
                {children}

            </motion.div>

            <motion.div ref={slideOutRef} id="slide-in" initial={{ y: '100%', x: '100%' }} className="absolute left-0 top-0 h-[100dvh] w-full bg-black" >
